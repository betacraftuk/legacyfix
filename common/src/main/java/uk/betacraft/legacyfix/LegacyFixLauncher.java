...
//        }
    ...
    private static void launch() {
        ...
        if (minecraftAppletClassName != null && !launchEntry(minecraftAppletClassName)) {
            Logger.error("Failed to find explicitly specified applet class: \"" + minecraftAppletClassName + "\"");
            return;
        }

        if (mainClassName != null && !launchEntry(mainClassName)) {
            Logger.error("Failed to find explicitly specified main class: \"" + mainClassName + "\"");
            return;
        }

        for (String candidate : MAIN_CLASS_CANDIDATES) {
            boolean success = launchEntry(candidate);
            Logger.debug("Main candidate: " + candidate + ": " + success);
        ...
        Logger.error("Failed to find the starting Minecraft class!");
    }
    ...
    public static String getGameDir() {
        ...
        if (gameDir == null) {
            try {
                ...
                Logger.error("LegacyFixLauncher", t);
                return ".";
            }
        }
